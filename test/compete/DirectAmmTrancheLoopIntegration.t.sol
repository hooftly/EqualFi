...
contract DirectAmmTrancheLoopIntegrationTest is DirectDiamondTestBase {
    ...
    uint64 internal constant TERM_SECONDS = 365 days;
    ...
    function test_LoopTrancheOfferSwapAndDeposit() public {
        ...
        uint256 offerId = _postTrancheOffer(collateralPerFill);
        ...
        DirectTypes.DirectTrancheView memory tranche = views.getOfferTranche(offerId);
        assertEq(tranche.trancheRemaining, 0, "tranche depleted");
        assertTrue(tranche.isDepleted, "tranche marked depleted");
    }
    ...
    function _postTrancheOffer(uint256 collateralLockAmount) internal returns (uint256 offerId) {
        ...
        DirectTypes.DirectTrancheOfferParams memory trancheParams = DirectTypes.DirectTrancheOfferParams({
            isTranche: true,
            trancheAmount: BORROW_AMOUNT * 3
        });

        vm.prank(userB);
        offerId = offers.postOffer(params, trancheParams);
    }
